...
　제가 진짜로 우려하는 것은 두 번째 관점입니다. **앞서 언급한 것처럼 자산의 변동성 제거는 무료가 아닙니다.** `Aperture Finance`에 많은 유동성이 몰려 `Mirror` 합성자산의 예치 보상이 그대로인 상태에서 예치된 유동성이 증가하면 `Mirror`의 예치 수익이 감소합니다. **그 결과 경우에 따라 수익이 `Anchor`보다 오히려 낮아질 수도 있습니다.** 자산의 2/3만 `Anchor`로 들어가기 때문입니다.

*(비슷한 사례로 일부 페어가 마이너스 수익을 기록한 적이 있었던 클레이스왑 플러스 예치가 있습니다. 그러나, `Aperture Finance`는 구조상 마이너스 수익까지는 가지 않을 것으로 보입니다.)*

　그리고 이는 반대로 **합성자산의 가격 하락으로 인한 청산 걱정이 없다**는 뜻이기도 합니다. 한 번 넣고 신경 쓰고 싶지 않다면, 미래 전망이 어두운 합성자산을 기반으로 하는 Delta-Neutural 전략에 투자하는 것이 괜찮아 보입니다.

　그리고 당연한 이야기지만, 위의 모든 전략은 `Aperture Finance`가 컨트랙을 잘 작성했다는 것을 전제로 합니다. 컨트랙이 복잡해질수록 취약점이 발생할 확률은 높아질 것입니다.
